...
Shell 是真正“理解”并“执行”你输入的命令的程序，它提供了多种功能。（[[Shell Tools]]）
Shell 脚本语言是一种解释型语言，可将一系列命令（ CLI 中输入的）写入一个文本文件中，然后让 Shell 程序按照脚本内容执行，这就是 [[Shell Scripting]]。
...
### 基本用法
1. **在文件中搜索指定模式的文本**： `grep` 最核心的功能是从一个或多个文件中查找匹配指定“模式”（通常是[[正则表达式]]）的行，并将这些行打印到标准输出。
```sh
