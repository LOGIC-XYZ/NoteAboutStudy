radix sort：是一种**非比较排序**，适用于**整数或定长字符串**排序。通过**按位排序**，将整数按“位”从低到高（或从高到低）逐位比较并排序。适用于数值范围较大的情况（相较于[[02 - study/Data Structures and  Algorithms/排序/计数排序]]，但是数据必须可以表示为固定位数的格式，且位数不能过大）。
