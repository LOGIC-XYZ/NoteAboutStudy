...
```
### 邻接表
Adjacency List存储方法是用数组与链表相结合来表示，以时间换空间。（ps：与[[NoteAboutStudy/Data Structures and  Algorithms/哈希表/哈希表]]中链式地址相似，故可类似优化）
...
### 边集数组
由两个一维数组构成的。一个存储顶点的信息；另一个存储边的信息，这个边数组每个数据元素由一条边的起点下标（begin）、终点下标（end）和权（weight）组成。（适合对边依次处理而不适合对顶点相关的操作） -- 与[[NoteAboutStudy/Data Structures and  Algorithms/图/Kruskal算法]]有关![[other/attachments/bdef512aaa9b1fff202c7aea0237f040.jpg]]

## 图的遍历
Traversing Graph 从图中某一顶点出发访遍图中其余顶点，且使每个顶点仅被访问一次。
为了防止重复遍历顶点，借助一个[[NoteAboutStudy/Data Structures and  Algorithms/哈希表/哈希集合]] `visited` 来记录哪些节点已被访问。
### 深度优先遍历（搜索）
...
Minimum Cost Spanning Tree为构造连通网的最小代价生成树
 ...
 - **[[NoteAboutStudy/Data Structures and  Algorithms/图/Prim算法]]** -- 从一个起点出发，不断扩展最短边（稠密图）
 - [[NoteAboutStudy/Data Structures and  Algorithms/图/Kruskal算法]] -- 按照边的权值从小到大逐一选边（稀疏图）

## 最短路径
对于网图来说，最短路径，是指两顶点之间经过的边上权值之和最少的路径，并且称路径上的第一个顶点是源点，最后一个顶点是终点
 - [[NoteAboutStudy/Data Structures and  Algorithms/图/Dijkstra算法]] -- 逐步扩展，依次找到源点到每个顶点的最短路径（贪心）
 - [[NoteAboutStudy/Data Structures and  Algorithms/图/Floyd算法]] -- 逐步检查每个顶点作为“中间点”的情况，可求任意两点最短路径（动态规划）

## [[NoteAboutStudy/Data Structures and  Algorithms/图/拓扑排序]]
拓扑排序是一种对**有向无环图 (DAG)** 进行的排序。它将图中的**顶点**排成一个线性序列，使得： **若存在一条从顶点 `u` 到顶点 `v` 的有向边 `(u → v)`，则在排序中 `u` 必须出现在 `v` 之前。**
## [[NoteAboutStudy/Data Structures and  Algorithms/图/关键路径]]
