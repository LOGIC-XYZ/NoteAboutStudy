## 引入
为了减少朴素匹配里不必要的回溯，所以需要在查找字符串前，先对要查找的字符串做一个分析，以减少查找的难度并提升速度，为此引入了 `next` 数组来跳到可以进行下一步匹配的位置（省略其中不必要的回溯）存储 `k` 值的就是 `next` 数组，前后缀一个字符相等，`k` 值是 2，两个字符相等 `k` 值是3，`n` 个字符相等，`k` 值就是 `n` -- 此时前缀与后缀相等的最大长度
